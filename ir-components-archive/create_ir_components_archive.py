...
id_to_component = flatten_tirex_components_to_id(yaml.load(open('ir-components-archive/links.yml').read(), Loader=yaml.FullLoader))
...
def get_snippet_to_run_components(component):
    ...
    # Load the dataset via ir-datasets
...
if __name__ == '__main__':
    components = yaml.load(open('ir-components-archive/links.yml').read(), Loader=yaml.FullLoader)
    components = flatten_components(components)
    json.dump(components, open('ui/src/components/links.json', 'w'), indent=2, sort_keys=True)
